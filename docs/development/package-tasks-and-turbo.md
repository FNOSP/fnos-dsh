...
flowchart TD
  ...
  invoke --> filter --> depGraph --> hash --> cache --> cacheStatus
  cacheStatus -->|命中| restore
  cacheStatus -->|未命中| deps
...
- `docs` 包的 `build` 用包级 `turbo.json` 覆盖 `outputs` 为 `.vitepress/dist/**`：VitePress 的实际产物不在 `dist/`，不覆盖会让 Turbo 报「no output files found」，文档产物既不入缓存也无法恢复。
...
flowchart TD
  ...
  webEnv["仓库根 .dsh 作为 DSH_HOME，端口 8070"]
...
flowchart TD
  ...
  turbo["turbo run typecheck --filter=目标包"]
  depGraph["解析 workspace 依赖"]
  ...
  command --> turbo --> depGraph --> deps --> target --> status
  status -->|是| result
  status -->|否| fail
```
...
flowchart TD
  ...
  command --> turbo --> depGraph --> build --> run --> status
  status -->|是| result
  status -->|否| fail
```
