...
本项目维护飞牛 fnOS 的 DeepSeek Harness 应用打包配置。项目/FPK 版本由根目录 `version` 任务和 `bumpp` 维护，插件版本独立处理，具体运行依赖以应用的 `manifest` 为准。
...
应用可以从 [GitHub Releases](https://github.com/tnnevol/fn-os-apps/releases) 下载 `.fpk` 文件后，在 fnOS 应用中心手动安装；开发调试时可使用 `appcenter-cli install-local`。
